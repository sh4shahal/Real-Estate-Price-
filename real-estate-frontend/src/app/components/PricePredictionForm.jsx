...
export function PricePredictionForm() {
  ...
  // Get addPrediction from shared context
  ...
  const handlePredict = async () => {
    try {
      ...
      const predictedPrice = res.data.predicted_price;

      setLockedLocation(form.location);
      setPrice(predictedPrice);
      setPredicted(true);

      // ✅ Push result into the shared analytics context
      addPrediction({
            ...
            {!predicted ? (
              ...
              <motion.div key="result" className="space-y-4">
                ...
                <p className="text-[10px] text-center text-blue-600 font-medium">
                   Analytics dashboard updated! Scroll down → Analytics tab to view.
                </p>
              </motion.div>
            )}
          </AnimatePresence>
